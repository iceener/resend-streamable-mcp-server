...
function parseScheduleTime(input: string): Date | null {
  ...
  const inMatch = lower.match(/^in\s+(\d+)\s*(min|minute|minutes|hour|hours|day|days)$/);
  if (inMatch) {
 ...
 */
function processContent(body: string, format: 'text' | 'html' | 'auto'): { html?: string; text?: string } {
  if (format === 'text') {
...
const InputSchema = z.object({
  ...
  to: z.union([z.string().email(), z.array(z.string().email())])
    .optional()
    .describe('Recipient email(s) for individual send. Single email or array up to 50. Use this OR "segment", not both.'),
  segment: z.string()
    .optional()
    .describe('Segment name for broadcast (sends to all contacts in segment). Use this OR "to", not both. Case-insensitive.'),
  ...
  format: z.enum(['text', 'html', 'auto'])
    .optional()
    .default('auto')
    .describe('Content format: "text" = plain text (best deliverability), "html" = HTML, "auto" = detect (plain text with newlines stays as text, HTML passes through). Default: auto'),
  template: z.string()
    .optional()
    .describe('Template ID or alias to use. Must be a published template. Use this OR "body", not both.'),
  variables: z.record(z.union([z.string(), z.number()]))
    .optional()
    .describe('Variables to inject into template, e.g. {"CTA_TEXT": "Sign Up", "DISCOUNT": 20}'),
  
  // Common fields
  subject: z.string()
    .optional()
    .describe('Email subject line. Required for broadcast. For templates, overrides template default if provided.'),
  name: z.string()
    .optional()
    .describe('Broadcast name for dashboard display (only for segment broadcasts). Defaults to subject if not provided.'),
  from_name: z.string()
    .optional()
    .describe('Sender display name, e.g. "Alice Newsletter". Appears as "Alice Newsletter <noreply@...>"'),
  reply_to: z.string().email()
    .optional()
    .describe('Reply-to email address. Recipients who reply will send to this address.'),
...
export const sendTool = defineTool({
  ...
  handler: async (args, context: ToolContext): Promise<ToolResult> => {
    ...
    if (!args.to && !args.segment) {
      return {
        content: [{ type: 'text', text: 'Error: Provide either "to" (email addresses) for individual send or "segment" (segment name) for broadcast. One is required.' }],
        isError: true,
      };
    }
    ...
    if (args.to && args.segment) {
      return {
        content: [{ type: 'text', text: 'Error: Cannot use both "to" and "segment". Choose individual send (to) or broadcast (segment).' }],
        isError: true,
      };
    }
    ...
    if (!args.body && !args.template) {
      return {
        content: [{ type: 'text', text: 'Error: Provide either "body" (HTML content) or "template" (template ID/alias). One is required.' }],
        isError: true,
      };
    }
    ...
    if (args.body && args.template) {
      return {
        content: [{ type: 'text', text: 'Error: Cannot use both "body" and "template". Choose one content source.' }],
        isError: true,
      };
    }
    ...
    if (!config.RESEND_DEFAULT_FROM) {
      return {
        content: [{ type: 'text', text: 'Error: RESEND_DEFAULT_FROM not configured. Set it in environment (e.g., newsletter@yourdomain.com).' }],
        isError: true,
      };
    }
    ...
    const fromAddress = args.from_name 
      ? `${args.from_name} <${config.RESEND_DEFAULT_FROM}>` 
      : config.RESEND_DEFAULT_FROM;
    ...
    if (args.to) {
      ...
      if (recipients.length > 50) {
        return {
          content: [{ type: 'text', text: 'Error: Maximum 50 recipients per individual send. Use broadcast for larger sends.' }],
          isError: true,
        };
      }
    ...
    if (args.segment) {
      ...
      if (!args.subject) {
        return {
          content: [{ type: 'text', text: 'Error: "subject" is required for broadcast. Provide an email subject line.' }],
          isError: true,
        };
      }
      ...
      if (args.schedule_for) {
        ...
        if (scheduledDate && scheduledDate < minTime) {
          return {
            content: [{ type: 'text', text: 'Error: Broadcasts must be scheduled at least 5 minutes ahead. This gives you time to cancel if needed. Use schedule_for="in 5 minutes" or later.' }],
            isError: true,
          };
        }
      }
