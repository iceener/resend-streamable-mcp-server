...
const InputSchema = z.object({
  ...
  campaign_id: z
    ...
    .optional()
    .describe(
      'Campaign/broadcast ID. Required for "status" and "cancel" actions. Get IDs from "list" action or from "send" tool response.',
    ),
  limit: z
    .number()
    .int()
    ...
    .max(100)
    .optional()
    .describe('Number of campaigns to return for "list" action. Default 20, max 100.'),
  cursor: z
    .string()
    .optional()
    .describe(
    ...
    switch (args.action) {
      case 'list': {
        ...
        const structured = CampaignsListOutputSchema.parse({
          ...
          cursor:
            response.has_more && items.length > 0
        ...
        if (items.length === 0) {
          return {
            content: [
              {
                type: 'text',
                text: 'No campaigns found. Use "send" tool with segment parameter to create a broadcast campaign.',
              },
            ],
        ...
        const campaignList = items
          .map((c) => {
            const segmentInfo = c.segment_name ? ` → ${c.segment_name}` : '';
            const scheduleInfo = c.scheduled_at
        ...
        if (!args.campaign_id) {
          return {
            content: [
              {
                type: 'text',
                text: 'Error: "campaign_id" is required for status action. Get IDs from action="list" or from "send" tool response.',
              },
            ],
            isError: true,
          };
        ...
        if (!args.campaign_id) {
          return {
            content: [
              {
                type: 'text',
                text: 'Error: "campaign_id" is required for cancel action. Get IDs from action="list".',
              },
            ],
            isError: true,
          };
        ...
        try {
          ...
          return {
            content: [
              {
                type: 'text',
                text: `Campaign ${args.campaign_id} cancelled successfully.`,
              },
            ],
            structuredContent: structured,
          };
          ...
          return {
            content: [
              {
                type: 'text',
                text: `Failed to cancel: ${(error as Error).message}. Only draft or scheduled campaigns can be cancelled. Already-sent campaigns cannot be undone.`,
              },
            ],
            isError: true,
          };
      ...
      default:
        return {
          content: [
            { type: 'text', text: 'Unknown action. Use: list, status, or cancel.' },
          ],
          isError: true,
        };
    }
